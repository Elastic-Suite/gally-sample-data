...
abstract class AbstractEntityGenerator extends AbstractGenerator
{
    ...
    public function generateAll(GenerationConfig $config, GenerationResult $result, OutputInterface $output): void
    {
        ...
        foreach ($this->catalogRepository->findAll() as $catalog) {
            ...
            // installIndexByName() refreshes the index and switches the alias right away,
            // so all pending async bulks must be resolved before that to avoid missing documents.
            $this->flushPendingBulks();
    ...
    /**
     * Percentage chance (0-100) that a non-system source field gets a value on a generated document;
     * All non product metadata will have a value for each sourceField.
     */
    ...
    private function flushPendingBulks(): void
    {
        if (0 === $this->parrallelBulkCount) {
            return;
        }
